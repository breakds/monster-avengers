...
#include <cstring>
#include <functional>
#include "dataset/base/properties.h"
...
struct Signature {
  ...
  inline Signature(const Armor &armor, const std::vector<Effect> &effects) 
      : Signature() {
    if (Arsenal::IsTorsoUp(armor)) {
      // Torso up armors are not allowedto have slots and effects.
      bytes[2] = 1;
      return;
    ...
    if (1 == armor.slots) {
      bytes[0] = 1;
    } else if (2 == armor.slots) {
      bytes[1] = 1;
    } else if (3 == armor.slots) {
      bytes[1] = 16;
    }
  ...
  inline Signature(const Jewel &jewel, 
    ...
    *valid = false;
    ...
    if (1 == jewel.slots) {
      bytes[0] = 1;
    } else if (2 == jewel.slots) {
      bytes[1] = 1;
    } else if (3 == jewel.slots) {
      bytes[1] = 16;
    }
  ...
  void ShowMetaInfo() const {
    ...
    int three = bytes[1] >> 4;
    wprintf(L"(%d %d %d) + %d, %d\n",
            one, two, three, BodySlotSum(), multiplier());
  }
  ...
  void BodySlots(int *i, int *j, int *k) const {
...
inline std::vector<Effect> KeyEffects(Signature key, 
  ...
  for (int i = 0; i < required.size(); ++i) {
    result.emplace_back();
    result.back().id = required[i].id;
    result.back().points = bytes[byte_id++];
  }
  return result;
}
...
inline void KeySlots(Signature key, 
...
inline Signature SlotsToKey(int one, int two, int three) {
  Signature key;
  char *bytes = reinterpret_cast<char*>(&key);
  bytes[0] = one;
  bytes[1] = two;
