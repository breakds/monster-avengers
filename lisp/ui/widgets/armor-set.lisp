...
(def-widget summary-table (language summary full-summary)
    ()
  #jsx(:table ((class-name "table table-bordered"))
              (:tbody ()
                      (:tr () 
                      ...
                      (chain summary 
                             (sort (lambda (x y)
                                     (if (< (@ x points) (@ y points)) 1 -1)))
                             (map (lambda (effect)
                                    (:tr ()
                                         (:td () (lang-text ("zh" (@ effect name jp))
                                                            ("en" (@ effect name en))))
                                         (:td () (@ effect points))
                                         (:td ((class-name (if (> (@ effect active en length) 0) 
                                                               "warning" "")))
                                              (lang-text ("zh" (@ effect active jp))
                                                         ("en" (@ effect active en)))))))))))
...
(def-widget armor-set-display (language armor-set blacklist-callback filter-callback weapon)
    ((state (jewel-plan-id 0)
            (full-summary false))
     (toggle-full-summary ()
                          (let ((new-value (not (local-state full-summary))))
                            (chain this (set-state (create full-summary
                                                           new-value))))
                          nil)
                        ...
                        (:div ((class-name "col-md-9 col-sm-9 col-xs-3"))
                              (:summary-table ((:language language)
                                               ...
                                               (summary (chain (@ (aref (@ armor-set jewel-plans)
                                                               ...
                                                               summary
                                                               (filter (lambda (x)
                                                                         (or (local-state full-summary)
                                                                             (@ x active en length) 0)))))))
                              (:button ((class-name "btn btn-info btn-xs")
                                        (on-click (@ this toggle-full-summary)))
                                       (lang-text ("en" "Toggle Full/Active Skill Summary")
                                                  ("zh" "切换显示 全部/发动 技能")))))
                  (:div ()
                        (:nav () 
